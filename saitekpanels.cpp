...
void process_radio_panel() {
  for (std::list<Radio *>::iterator it = radios.begin(); it != radios.end(); it++) {
    (*it)->update();
  }
}
			    ...
			    char *		outDesc)
{
  ...
  XPLMDebugString("Xsaitekpanels v1.46\n");

	/* First set up our plugin info. */
  strcpy(outName, "Xsaitekpanels v1.46");
...
// ************* Open any Radio that is connected *****************
  ...
  while (rad_cur_dev) {
    hid_device *handle = hid_open_path(rad_cur_dev->path);
    hid_set_nonblocking(handle, 1);

    Radio *r = new Radio(handle);
    r->update();
    radios.push_back(r);
